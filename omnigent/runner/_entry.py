...
def _make_auth_token_factory(
    ...
    *,
    ...
    if _allow_delegated_mint and delegated_auth and resolved_server_url and binding_token:
        # The host bearer doubles as the Apps-proxy credential for the mint
        # request itself, so the mint can be tried while we still hold it.
        delegated_factory = _make_managed_mint_factory(
            resolved_server_url,
            binding_token,
            proxy_bearer=_proxy_bearer or initial_token or None,
        )
