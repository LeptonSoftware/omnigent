...
def create_hosts_router(
    ...
    *,
    ...
    async def get_host_model_options(
        ...
        """
        user_id = require_user(request, auth_provider)
        # A shared user launches sessions on this host, so they need its
        # model catalog to pick from — "use", the same level the launch
        # itself requires. Installing harnesses and touching credentials
        # stay owner-only below.
        host = await asyncio.to_thread(
            resolve_host_access,
            user_id=user_id,
            host_id=host_id,
