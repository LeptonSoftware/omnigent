    ...
    *,
    ...
    async def list_hosts(request: Request) -> dict[str, list[dict[str, Any]]]:
        ...
        if user_id is None:
            ...
            # Fork: shared team hosts — owned + shared with the caller.
            ...
            result.append(
                {
                    ...
                    "name": host.name,
                    "owner": host.user_id,
                    # Fork: lets the picker label a shared host and hide
                    # owner-only affordances.
                    "is_owned_by_me": user_id is None or host.user_id == user_id,
    ...
    async def get_host(request: Request, host_id: str) -> dict[str, Any]:
        ...
        # Fork: shared team hosts — owner OR a grantee at this level.
        if not host_access_allowed(host, user_id, host_store, HOST_LEVEL_READ):
    ...
    async def get_host_model_options(
        ...
        if host is None:
            raise HTTPException(status_code=404, detail="host not found")
        # Fork: shared team hosts — owner OR a grantee at this level.
        if not host_access_allowed(host, user_id, host_store, HOST_LEVEL_USE):
            raise HTTPException(status_code=403, detail="not your host")
