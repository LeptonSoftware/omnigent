...
# ── Server-side session titles ──────────────────────────────────────────────
#
# The title gateway names sessions from the server rather than the runner (see
# ``omnigent.server.title_gateway``). It must be a NON-REASONING model: a title
# is capped at 32 output tokens and a reasoning model spends that whole budget
# thinking, returning empty content.

_TITLE_GATEWAY_FALLBACK = StaticModelFallback(
    model_ids=("anthropic/claude-haiku-4.5",),
    owner="Server-side session titles (omnigent.server.title_gateway)",
    provenance="AI-gateway model slug, verified to answer within the title token cap",
    discovery_gap="the OpenAI-compatible gateway exposes no model-listing API to the server",
)


def title_gateway_model() -> str:
    """Return the default model id for server-side session titles."""
    return _TITLE_GATEWAY_FALLBACK.model_ids[0]
