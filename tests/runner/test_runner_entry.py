...
def test_initial_host_token_defers_local_auth_until_rejected(
    ...
    def _declining_mint(*args: Any, **kwargs: Any) -> tuple[str, float]:
        del args, kwargs
        mint_calls.append(1)
...
def test_initial_host_token_falls_back_to_managed_mint_when_no_sdk_auth(
    ...
    """A managed runner authenticates by managed mint, never with no token.

    When no SDK/OIDC credential is available (managed sandbox with no user
    credential), the runner must still reach the managed-mint path rather than
    returning None and bricking all HTTP callbacks.

    Fork divergence: upstream returns the host bearer first and only reaches
    the mint after that bearer is rejected. This fork prefers the owner-bound
    mint up front (see ``test_owner_mint_preferred_over_host_bearer``), so the
    mint is reached directly — the property under test is unchanged.
    """
    mint_calls: list[int] = []
    ...
    # Every callback authenticates with the minted owner token — the runner is
    # never left unauthenticated, which is what this test exists to protect.
    request = httpx.Request("GET", "https://app.databricksapps.com/api/version")
    redirect = httpx.Response(302, headers={"Location": "/oidc/oauth2/v2.0/authorize"})
    captured = _drive_auth_flow(_RunnerDatabricksAuth(factory), request, redirect)
